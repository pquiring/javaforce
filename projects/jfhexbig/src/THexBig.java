/** THex - text/ansi hex editor (big edition)
 *
...
public class THexBig implements KeyEvents {
  ...
  public int menusPos[] = {2, 7, 12, 18, 25};
...
//    System.err.println("keyTyped:" + (int)key);
  ...
  public static class Tab {
    ...
    public void delete(int off1, int off2) {
      for(int off=off1;off<=off2;off++) {
        data[off] = 0x00;
      }
    }
    public void deleteSelection(int height) {
      ...
      if (sel_begin_y == sel_end_y) {
        ...
        delete(pos + sel_begin_x, pos + sel_end_x);
      } else {
        delete(sel_end_y * 16, sel_end_y * 16 + sel_end_x);
        delete(sel_begin_y * 16 + sel_begin_x, sel_begin_y * 16 + 16);
      }
      cy = sel_begin_y;
      cx = sel_begin_x;
    ...
    public byte[] getRange(int off1, int off2) {
      int length = off2 - off1 + 1;
      byte[] range = new byte[length];
      System.arraycopy(data, off1, range, 0, length);
      return range;
    }
    public byte[] getSelection() {
      if (!hasSelection()) return null;
      JFArrayByte buf = new JFArrayByte();
      for(int ln=sel_begin_y;ln <= sel_end_y;ln++) {
        if (ln == sel_begin_y) {
          int pos = sel_begin_y * 16;
          if (sel_begin_y != sel_end_y) {
            buf.append(getRange(pos + sel_begin_x, pos + 16));
          } else {
            buf.append(getRange(pos + sel_begin_x, pos + sel_end_x));
          }
        } else if (ln == sel_end_y) {
          int pos = sel_end_y * 16;
          buf.append(getRange(pos, pos + sel_end_x));
        } else {
          int pos = ln * 16;
          buf.append(getRange(pos, pos + 16));
        }
      }
      return buf.toArray();
    }
    ...
    public int indexOf(byte[] buf, int pos) {
      int start_pos = pos;
      int end_pos = length - pos - buf.length;
      if (end_pos < start_pos) return -1;
    ...
    public boolean writeBlock() throws Exception {
  ...
  public void drawTab() {
    ...
    for(int y=0;y<h3;y++) {
      ...
      if (pos < 512) {
        ...
        for(int x=0;x<16;x++) {
          if (pos < length) {
            ln.append(String.format("%02x", data[pos]));
          } else {
            ln.append("  ");
          }
          ln.append(' ');
  ...
  */
  ...
  public void copy() {
    ...
    Tab tab = tabs.get(tabidx);
    String str = new String(tab.getSelection());
    if (str == null) return;
  ...
  public void doFind(boolean showError) {
    ...
    int idx = tab.indexOf(find.getBytes(), start_pos);
    if (idx != -1) {
      found = true;
