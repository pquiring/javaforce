...
struct GLFWContextJNI {
  GLFWwindow *window;
  JNIEnv *ge;  //Warning : JNIEnv are thread specific
  jobject obj;
  jmethodID mid_dispatch;
};

static void key_callback_jni(GLFWwindow* window, int key, int scancode, int action, int mods)
{
  GLFWContextJNI *ctx = (GLFWContextJNI*)glfwGetWindowUserPointer(window);
  int type;
  if (action > 0)
    type = javaforce_jni_UIJNI_KEY_PRESS;
  else
    type = javaforce_jni_UIJNI_KEY_RELEASE;
  ctx->ge->CallVoidMethod(ctx->obj, ctx->mid_dispatch, type, key, 0);
}

static void character_callback_jni(GLFWwindow* window, unsigned int codepoint)
{
  GLFWContextJNI *ctx = (GLFWContextJNI*)glfwGetWindowUserPointer(window);
  ctx->ge->CallVoidMethod(ctx->obj, ctx->mid_dispatch, javaforce_jni_UIJNI_KEY_TYPED, codepoint, 0);
}

static void cursor_position_callback_jni(GLFWwindow* window, double xpos, double ypos)
{
  GLFWContextJNI *ctx = (GLFWContextJNI*)glfwGetWindowUserPointer(window);
  ctx->ge->CallVoidMethod(ctx->obj, ctx->mid_dispatch, javaforce_jni_UIJNI_MOUSE_MOVE, (int)xpos, (int)ypos);
}

static void mouse_button_callback_jni(GLFWwindow* window, int button, int action, int mods)
{
  GLFWContextJNI *ctx = (GLFWContextJNI*)glfwGetWindowUserPointer(window);
  int type;
  if (action > 0)
  ...
  ctx->ge->CallVoidMethod(ctx->obj, ctx->mid_dispatch, type, ++button, 0);
}

static void scroll_callback_jni(GLFWwindow* window, double xoffset, double yoffset)
{
  GLFWContextJNI *ctx = (GLFWContextJNI*)glfwGetWindowUserPointer(window);
  ctx->ge->CallVoidMethod(ctx->obj, ctx->mid_dispatch, javaforce_jni_UIJNI_MOUSE_SCROLL, (int)-xoffset, (int)-yoffset);
}

static void window_close_callback_jni(GLFWwindow* window)
{
  GLFWContextJNI *ctx = (GLFWContextJNI*)glfwGetWindowUserPointer(window);
  ctx->ge->CallVoidMethod(ctx->obj, ctx->mid_dispatch, javaforce_jni_UIJNI_WIN_CLOSING, 0, 0);
}

static void window_size_callback_jni(GLFWwindow* window, int width, int height)
{
  GLFWContextJNI *ctx = (GLFWContextJNI*)glfwGetWindowUserPointer(window);
  ctx->ge->CallVoidMethod(ctx->obj, ctx->mid_dispatch, javaforce_jni_UIJNI_WIN_RESIZE, width, height);
}
  ...
  (JNIEnv *e, jobject c, jint style, jstring title, jint x, jint y, jobject events, jlong shared)
{
  ...
  if (shared != 0) {
    GLFWContextJNI *sharedCtx = (GLFWContextJNI*)shared;
  ...
  glfwSetMouseButtonCallback(ctx->window, mouse_button_callback_jni);
  glfwSetScrollCallback(ctx->window, scroll_callback_jni);
  glfwSetWindowCloseCallback(ctx->window, window_close_callback_jni);
  glfwSetWindowSizeCallback(ctx->window, window_size_callback_jni);
