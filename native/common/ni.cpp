...
JNIEXPORT jboolean JNICALL Java_javaforce_controls_ni_DAQmx_createChannelAnalog
  (JNIEnv *e, jclass cls, jlong task, jstring dev, jdouble min, jdouble max)
{
...
JNIEXPORT jboolean JNICALL Java_javaforce_controls_ni_DAQmx_createChannelDigital
  (JNIEnv *e, jclass cls, jlong task, jstring dev)
{
  TaskHandle taskHandle=(TaskHandle)task;
  int32 status = 1;
  const char *_dev = e->GetStringUTFChars(dev, NULL);
...
JNIEXPORT jboolean JNICALL Java_javaforce_controls_ni_DAQmx_createChannelCounter
  (JNIEnv *e, jclass cls, jlong task, jstring dev, jdouble min, jdouble max, jdouble measureTime, jint divisor)
{
  TaskHandle taskHandle=(TaskHandle)task;
  int32 status = 1;
  const char *_dev = e->GetStringUTFChars(dev, NULL);
  ...
  (JNIEnv *e, jclass cls, jlong task, jstring dev, jdouble rate, jlong samples)
{
  ...
  int32 status = 1;
  const char *_dev = "";
  if (dev != NULL) _dev = e->GetStringUTFChars(dev, NULL);
  status = (*_DAQmxCfgSampClkTiming)(taskHandle, _dev, rate, DAQmx_Val_Rising, DAQmx_Val_ContSamps, samples);
  if (dev != NULL) e->ReleaseStringUTFChars(dev, _dev);
  return status == 0;
}
  ...
  (JNIEnv *e, jclass cls, jlong task, jintArray data)
{
  ...
  void *_data = e->GetIntArrayElements(data, NULL);
  int size = e->GetArrayLength(data);
  (*_DAQmxReadCounterU32)(taskHandle, size, 10.0, (uInt32*)_data, size, &read, NULL);
  e->ReleaseIntArrayElements(data, (jint*)_data, JNI_COMMIT);
  return read;
}
